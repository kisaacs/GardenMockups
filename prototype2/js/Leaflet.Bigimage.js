...
(function (factory, window) {
    ...
    L.Control.BigImage = L.Control.extend({
        ...
        _createControl: function (label, title, classesToAdd, fn, context) {
            ...
            this._createDownloadButton("PNG");
            this._createDownloadButton("PDF");
            this._createDownloadButton("CSV");
        ...
        _createDownloadButton: function (downloadTitle) {
            this._downloadBtn = document.createElement('div');
            this._downloadBtn.classList.add('download-button');
            this._downloadBtn.id = downloadTitle + this.options.mapName;
            this._downloadBtn.innerHTML = downloadTitle;

            if (downloadTitle == "PNG" || downloadTitle == "PDF") {
        ...
        _print: function (downloadTitle) {
            let self = this;
            let name = this.options.mapName;
            ...
            promise.then(() => {
                ...
                if (downloadTitle == "PNG") {
                    ...
                    var pageData = self.canvas.toDataURL('image/jpeg', 1.0);
                    var pdf = new jsPDF('l', 'pt', [self.canvas.width, self.canvas.height]);
                    pdf.addImage(pageData, 'JPEG', 0, 0, self.canvas.width, self.canvas.height);
                    pdf.save(name + '.pdf');
